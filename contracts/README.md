...
| `src/DisputeRegistry.sol` | Disputes a milestone claim on UMA in the caller's name and records the written reason's hash and link on-chain in the same transaction; keeps every reason per milestone. |
...
| `test/DisputeRegistry.t.sol` | 8 dispute-registry tests. |
| `test/SettlementReceiver.t.sol` | 8 receiver tests. |
| `test/GrantEscrow.fork.t.sol` | 8 tests against the real UMA oracle and Circle USDC on a Base Sepolia fork. |
...
| `script/DeployDisputeRegistry.s.sol` | Deploys the dispute registry beside an existing escrow. |
